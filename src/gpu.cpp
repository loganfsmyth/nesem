...
//	cout << "X:" << (int)cycle << " Y:" << (int)scanline << endl;
  ...
  if(cycle < 256 && scanline > 0 && scanline < 241 && (reg_mask&0x08) != 0 ){
    ...
    win->DrawPixel(2*cycle,2*(scanline-1)+1,(int)c.r,(int)c.g,(int)c.b);
...
//cout << hex << (int)((x >> 3)|((y >> 3) << 5 )|0x2000) << "\t" << pt_num << endl;
  //~ cout << dec << "(" << cycle << ", " << scanline << ") " << hex << " Reg:0x" << (int)vram_address_reg << " at " << (int)offset_x << "," <<  (int)y_offset <<" 0x" << (int)reg_ctrl<< " - 0x" << pt_num << " -> 0x" << po << " 0x"<< pt << dec<< endl;
  
  int patterndat_one = mem_read(po);
  int patterndat_two = mem_read(pt);
...
void GPU::render_sprites(){
  ...
  for(int i = 0; i < 0x100; i += 0x04){
	...
	int pix_color = mem_read(0x3F10|pix_index);
