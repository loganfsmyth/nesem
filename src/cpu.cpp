...
void CPU::reset(){
  PC = map->read(0xFFFC) | (map->read(0xFFFD) << 8);
}
...
void CPU::trigger_interrupt(){
  ...
  PC = map->read(0xFFFA) | (map->read(0xFFFB) << 8);
...
void CPU::print_stack(){
  ...
  int i;
  for(i = 0xFF; i > SP; i--){
    cout << (i + 0x100) << ": " << (int)map->read(i + 0x100) << endl;
  }
...
//  cout << "Step: 0x" << hex << (unsigned int)PC << endl;
...
//  cout << "Type: " << inst_info.type << endl;
  ...
  if(inst_info.length > 1) arg1 = map->read(PC + 1);
  if(inst_info.length > 2) arg2 = map->read(PC + 2);
...
//  uint16_t addr = get_address(inst_info.type, arg1, arg2);
  ...
  switch(inst){
    ...
    case BRK_IMP:
      ...
      PUSH(SR);
      SET_INTERRUPT((1));
      PC = (map->read(0xFFFE) | (map->read(0xFFFF) << 8));
      break;
    ...
    case JMP_AB:
      ...
      break;
    case JMP_IN:
      PC = map->read((arg2 << 8) | arg1) | (map->read(((arg2 << 8) | arg1) + 1) << 8);
      break;
...
void CPU::write(int type, uint8_t arg1, uint8_t arg2, uint8_t data){
  ...
  switch(type){
    ...
    case MEM_ZP:
      ...
      break;
    case MEM_ZPX:
      map->write((arg1 + (uint8_t)XR) & 0x00FF, data);
      break;
    case MEM_ZPY:
      map->write((arg1 + (uint8_t)YR) & 0x00FF, data);
      break;
    case MEM_AB:
      map->write((arg2 << 8) | arg1, data);
      break;
    case MEM_ABX:
      map->write(((arg2 << 8) | arg1) + (uint8_t)XR, data);
      break;
    case MEM_ABY:
      map->write(((arg2 << 8) | arg1) + (uint8_t)YR, data);
      break;
    case MEM_INX:
      lower = map->read(arg1 + (uint8_t)XR);
      upper = map->read(arg1 + (uint8_t)XR + 1);
      map->write((upper << 8) | lower, data);
      break;
    case MEM_INY:
      lower = map->read(arg1);
      upper = map->read(arg1 + 1);
      map->write(((upper << 8) | lower) + (uint8_t)YR, data);
      break;
    case MEM_IMP:
      break;
    case MEM_AC:
...
uint8_t CPU::read(int type, uint8_t arg1, uint8_t arg2){
  ...
  switch(type){
    ...
    case MEM_ZP:
      return map->read(arg1);

    case MEM_ZPX:
      return map->read((arg1 + (uint8_t)XR) & 0x00FF);

    case MEM_ZPY:
      return map->read((arg1 + (uint8_t)YR) & 0x00FF);

    case MEM_AB:
      return map->read((arg2 << 8) | arg1);

    case MEM_ABX:
      return map->read(((arg2 << 8) | arg1) + (uint8_t)XR);

    case MEM_ABY:
      return map->read(((arg2 << 8) | arg1) + (uint8_t)YR);

    case MEM_INX:
      lower = map->read(arg1 + (uint8_t)XR);
      upper = map->read(arg1 + (uint8_t)XR + 1);
      return map->read((upper << 8) | lower);
    ...
    //cout << "INY 0x" << hex << (unsigned int)(((upper << 8) | lower) + (uint8_t)YR) << dec << endl;
      ...
      return map->read(((upper << 8) | lower) + (uint8_t)YR);
