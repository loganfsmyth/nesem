<html><head>
<meta http-equiv="content-type" content="text/html; charset=ISO-8859-1"><title>hexdump</title></head><body>/*
...
html += "<table class="screen"><tbody><tr><td class="screen" id="x' + x + 'y' + y + '"></td></tr></tbody></table>";
...
function compileCode() {
  ...
  for( xc=0; xc<lines.length; xc++="" if(="" !="" indexlabels(="" lines[xc]="" )="" {="" message(=""><b>Label already defined at line "+(xc+1)+":</b> "+lines[xc] );
      return false;
    }
  }
  ...
  for( x=0; x<lines.length; x++="" !="" compileline(="" lines[x],="" x="" break;="" codelen="=" 0="" message(="" no="" code="" to="" run.="" );="" if(="" codecompiledok="false;" )="" runbutton="" hexdumpbutton="" compilebutton="" fileselect="" document.getelementbyid(="" submitcode="" ).disabled="false;" memory[defaultcodepc]="0x00;" }="" else="" {="" str="lines[x].replace(">&lt;", "&lt;" ).replace( "&gt;", "&gt;" );
...
function findLabel( name ) {
  for( m=0; m<labelindex.length; m++="" nameandaddr="labelIndex[m].split(" |="" );="" if(="" name="=" nameandaddr[0]="" true;="" return="" false;="" }="" setlabelpc()="" -="" associates="" label="" with="" address="" *="" function="" setlabelpc(="" name,="" addr="" )="" {="" for(="" i="0;"><labelindex.length; i++="" nameandaddr="labelIndex[i].split(" );="" if(="" nameandaddr[0]="" labelindex[i]="name" |="" +="" addr;="" true;="" return="" false;="" }="" getlabelpc()="" -="" get="" address="" associated="" with="" label="" *="" function="" getlabelpc(="" name="=" )="" {="" for(="" i="0;"><labelindex.length; i++="" nameandaddr="labelIndex[i].split(" |="" name="=" nameandaddr[0]="" (nameandaddr[1]);="" -1;="" compileline()="" compiles="" one="" of="" code.="" returns="" true="" if="" it="" compiled="" successfully,="" false="" otherwise.="" *="" function="" compileline(="" input,="" lineno="" remove="" comments="" ^(.*?);.*="" trim="" line="" ^\s+="" \s+$="" find="" or="" ^\w+:="" label="" (^\w+):.*$="" ^\w+:[\s]*\w+.*$="" input="input.replace(" ^\w+:[\s]*(.*)$="" ;="" ^(\w+).*$="" $1="" blank="" line?="" return.="" return="" true;="" command="" input.match(="" ^\*[\s]*="[\s]*[\$]?[0-9a-f]*$/" equ="" spotted="" ^[\s]*\*[\s]*="[\s]*/" param[0]="=" $="" )="" param="input.replace(" new="" regexp(="" ^\$="" ),="" 16="" else="" {="" addr="parseInt(" param,="" 10="" );="" }="" if(="" (addr="">&lt; 0) || (addr &gt; 0xffff) ) {
  ...
  for( o=0; o<opcodes.length; o++="" opcodes[o][0]="=" command="" checksingle(="" opcodes[o][10]="" checkimmediate(="" opcodes[o][1]="" checkzeropage(="" opcodes[o][2]="" checkzeropagex(="" opcodes[o][3]="" checkzeropagey(="" opcodes[o][4]="" checkabsolutex(="" opcodes[o][6]="" checkabsolutey(="" opcodes[o][7]="" checkindirectx(="" opcodes[o][8]="" checkindirecty(="" opcodes[o][9]="" checkabsolute(="" opcodes[o][5]="" checkbranch(="" param,="" opcodes[o][11]="" true;="" unknown="" opcode="" }="" *****************************************************************************="" ****************************************************************************="" function="" dcb(="" param="" {="" values="param.split(" ,="" );="" if(="" values.length="=" 0="" )="" return="" false;="" for(="" v="0;"><values.length; v++="" )="" {="" if(="" str="values[v];" !="undefined" &&="" str.length=""> 0 ) {
...
function checkZeroPageY(
param, opcode ) { if( opcode == 0x00 ) return false; if( param.match(
/^\$[0-9a-f]{1,2},Y/i ) ) { pushByte( opcode ); number = param.replace(
new RegExp( /^\$([0-9a-f]{1,2}),Y/i ), "$1" ); value = parseInt(
number, 16 ); if( value &lt; 0 || value &gt; 255 ) return false;
pushByte( value ); return true; } if( param.match( /^[0-9]{1,3},Y/i ) )
{ pushByte( opcode ); number = param.replace( new RegExp(
/^([0-9]{1,3}),Y/i ), "$1" ); value = parseInt( number, 10 ); if( value
&lt; 0 || value &gt; 255 ) return false; pushByte( value ); return
true; } return false;
}
/* * checkAbsolute() - Check if param is ABS and push value * */
function checkAbsolute( param, opcode ) { if( opcode == 0x00 ) return
false; pushByte( opcode ); if( param.match( /^\$[0-9a-f]{3,4}$/i ) ) {
value = parseInt( param.replace( /^\$/, "" ), 16 ); if( value &lt; 0 ||
value &gt; 0xffff ) return false; pushWord( value ); return true; } //
it could be a label too.. if( param.match( /^\w+$/ ) ) { if( findLabel(
param ) ) { addr = (getLabelPC( param )); if( addr &lt; 0 || addr &gt;
0xffff ) return false; pushWord( addr ); return true; } else {
pushWord( 0x1234 ); return true; } } return false;
}
/*****************************************************************************
...
function stackPush( value ) { if( regSP &gt;= 0 ) { regSP--;
memory[(regSP&amp;0xff)+0x100] = value &amp; 0xff; } else { message(
"Stack full: " + regSP ); codeRunning = false; }
}
/*****************************************************************************
...
function stackPop() { if( regSP &lt; 0x100 ) { value =
memory[regSP+0x100]; regSP++; return value; } else { message( "Stack
empty" ); codeRunning = false; return 0; }
}
/* * pushByte() - Push byte to compiledCode variable * */
function pushByte( value ) { memory[defaultCodePC] = value &amp; 0xff;
defaultCodePC++; codeLen++;
}
/* * pushWord() - Push a word using pushByte twice * */
function pushWord( value ) { pushByte( value &amp; 0xff ); pushByte(
(value&gt;&gt;8) &amp; 0xff );
}
/* * popByte() - Pops a byte * */
...
function memStoreByte( addr, value ) { memory[ addr ] = (value &amp;
0xff); if( (addr &gt;= 0x200) &amp;&amp; (addr&lt;=0x5ff) )
display[addr-0x200].background = palette[memory[addr] &amp; 0x0f];
}
/* * memStoreByte() - Peek a byte, don't touch any registers * */
function memReadByte( addr ) { if( addr == 0xfe ) return Math.floor(
Math.random()*256 ); return memory[addr];
}
/* * submitCode() - Submits code (using XMLHttpRequest) to be published
(moderated) * */
function submitCode() { if( confirm( "Warning: This will submit your
code to 6502asm.com for moderation.\n" + "Approved code will be
published on the website." ) == false ) return; // Let's submit it
xmlhttp = new XMLHttpRequest(); xmlhttp.onreadystatechange = function()
{ if( xmlhttp.readyState==4 &amp;&amp; xmlhttp.status==200 ) { message(
"-- Thank you for sharing your code with other 6502asm.com users." );
message( "-- Your code has been submitted for moderation." ); message(
"-- Once approved, it will be published on the website." ); if(
xmlhttp.responseText != "" ) { alert( "An error occoured while
submitting your code. The error message was:\n" + xmlhttp.responseText
+ "\n" + "Please try again later." ); } } } var code =
document.getElementById( "code" ).value; var params = "code=" + code;
xmlhttp.open( "POST", "submit.php", true );
xmlhttp.setRequestHeader("Content-type",
"application/x-www-form-urlencoded");
xmlhttp.setRequestHeader("Content-length", params.length);
xmlhttp.setRequestHeader("Connection", "close"); xmlhttp.send( params
);
}
/* * hexDump() - Dump binary as hex to new window * */
function addr2hex( addr ) { return
num2hex((addr&gt;&gt;8)&amp;0xff)+num2hex(addr&amp;0xff);
}
function num2hex( nr ) { str = "0123456789abcdef"; hi =
((nr&amp;0xf0)&gt;&gt;4); lo = (nr&amp;15); return str.substring( hi,
hi+1 ) + str.substring( lo, lo+1 );
}
/*
function hexDump() { w = window.open('', 'hexdump',
'width=500,height=300,resizable=yes,scrollbars=yes,toolbar=no,location=no,menubar=no,status=no'
); html = ""; html += "<link href="assembler_files/style.css" rel="stylesheet" type="text/css">";
  html += "";
  html += "<code>";
  for( x=0; x<codelen; x++="" if(="" (x&15)="=" 0="" )="" {="" ;="" n="(0x600+x);" html="" +="&lt;br/&gt; " ((n="">&gt;8)&amp;0xff) );
      html += num2hex( (n&amp;0xff) );
      html += ": ";
    }
...
//      regPC = memReadByte(popByte()) + (memReadByte(popByte())&lt;&lt;8);
    ...
    case 0xb4:                          // LDY ZPX
      ...
      if( regY ) regP &amp;= 0xfd; else regP |= 0x02;
      if( regY
&amp; 0x80 ) regP |= 0x80; else regP &amp;= 0x7f; break; case 0xb5: //
LDA ZPX regA = memReadByte( (popByte() + regX) &amp; 0xff ); if( regA )
regP &amp;= 0xfd; else regP |= 0x02; if( regA &amp; 0x80 ) regP |=
0x80; else regP &amp;= 0x7f; break; case 0xb6: // LDX ZPY regX =
memReadByte( popByte() + regY ); if( regX ) regP &amp;= 0xfd; else regP
|= 0x02; if( regX &amp; 0x80 ) regP |= 0x80; else regP &amp;= 0x7f;
break; case 0xb8: // CLV regP &amp;= 0xbf; break; case 0xb9: // LDA
ABSY addr = popWord() + regY; regA = memReadByte( addr ); if( regA )
regP &amp;= 0xfd; else regP |= 0x02; if( regA &amp; 0x80 ) regP |=
0x80; else regP &amp;= 0x7f; break; case 0xba: // TSX regX = regSP
&amp; 0xff; break; case 0xbc: // LDY ABSX addr = popWord() + regX; regY
= memReadByte( addr ); if( regY ) regP &amp;= 0xfd; else regP |= 0x02;
if( regY &amp; 0x80 ) regP |= 0x80; else regP &amp;= 0x7f; break; case
0xbd: // LDA ABSX addr = popWord() + regX; regA = memReadByte( addr );
if( regA ) regP &amp;= 0xfd; else regP |= 0x02; if( regA &amp; 0x80 )
regP |= 0x80; else regP &amp;= 0x7f; break; case 0xbe: // LDX ABSY addr
= popWord() + regY; regX = memReadByte( addr ); if( regX ) regP &amp;=
0xfd; else regP |= 0x02; if( regX &amp; 0x80 ) regP |= 0x80; else regP
&amp;= 0x7f; break; case 0xc0: // CPY IMM value = popByte(); if(
(regY+value) &gt; 0xff ) regP |= 1; else regP &amp;= 0xfe; ov = value;
value = (regY-value); if( value ) regP &amp;= 0xfd; else regP |= 0x02;
if( value &amp; 0x80 ) regP |= 0x80; else regP &amp;= 0x7f; break; case
0xc1: // CMP INDY zp = popByte(); addr = memReadByte(zp) +
(memReadByte(zp+1)&lt;&lt;8) + regY; value = memReadByte( addr );
doCompare( regA, value ); break; case 0xc4: // CPY ZP value =
memReadByte( popByte() ); doCompare( regY, value ); break; case 0xc5:
// CMP ZP value = memReadByte( popByte() ); doCompare( regA, value );
break; case 0xc6: // DEC ZP zp = popByte(); value = memReadByte( zp );
--value; memStoreByte( zp, value&amp;0xff ); if( value ) regP &amp;=
0xfd; else regP |= 0x02; if( value &amp; 0x80 ) regP |= 0x80; else regP
&amp;= 0x7f; break; case 0xc8: // INY regY = (regY + 1) &amp; 0xff; if(
regY ) regP &amp;= 0xfd; else regP |= 0x02; if( regY &amp; 0x80 ) regP
|= 0x80; else regP &amp;= 0x7f; break; case 0xc9: // CMP IMM value =
popByte(); doCompare( regA, value ); break; case 0xca: // DEX regX =
(regX-1) &amp; 0xff; if( regX ) regP &amp;= 0xfd; else regP |= 0x02;
if( regX &amp; 0x80 ) regP |= 0x80; else regP &amp;= 0x7f; break; case
0xcc: // CPY ABS value = memReadByte( popWord() ); doCompare( regY,
value ); break; case 0xcd: // CMP ABS value = memReadByte( popWord() );
doCompare( regA, value ); break; case 0xce: // DEC ABS addr =
popWord(); value = memReadByte( addr ); --value; value =
value&amp;0xff; memStoreByte( addr, value ); if( value ) regP &amp;=
0xfd; else regP |= 0x02; if( value &amp; 0x80 ) regP |= 0x80; else regP
&amp;= 0x7f; break; case 0xd0: // BNE offset = popByte();
// if( (regP&amp;2)==0 ) { oldPC = regPC; jumpBranch( offset );
message( "Jumping from " + addr2hex(oldPC) + " to " + addr2hex(regPC)
); } else { message( "NOT jumping!" ); } if( (regP&amp;2)==0 )
jumpBranch( offset ); break;   case 0xd1:                          // CMP INDY
      zp = popByte();
      addr = memReadByte(zp) + (memReadByte(zp+1)&lt;&lt;8) + regY;
      value = memReadByte( addr );
...
function updateDisplayFull() {
 ...
 </values.length;></opcodes.length;></labelindex.length;></labelindex.length;></labelindex.length;></lines.length;></lines.length;></body></html>